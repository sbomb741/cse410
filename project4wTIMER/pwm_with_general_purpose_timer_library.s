...
; ==========================================================
; SUBROUTINE - PWM RGB DEINITIALIZATION
pwm_RGB_LED_deinit:
	PUSH {r4-r12, lr}

	; STEP 3~: Set GPIO pins to not use alternate function (standard GPIO)
	mov r4, #0x5000
	movt r4, #0x4002				; Port F Advanced Peripheral Bus (APB)
	ldrb r5, [r4, #GPIOAFSEL]
	mov r6, #0x0					; set pins 1-3 to not use alternate function (RGB pins)
	bfi r5, r6, #1, #3
	strb r5, [r4, #GPIOAFSEL]
	bl nops							; nop 5 times

	; STEP 9~: End the Timers
	mov r4, #0x9000
